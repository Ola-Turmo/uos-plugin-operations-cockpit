...
export class CockpitState {
  ...
  private healthRecords: Map<string, ToolHealthRecord>;
  ...
  getOverview(): CockpitOverview {
    ...
    return {
      ...
      openIncidents: allIncidents.filter(i => 
        i.status === IncidentStatus.OPEN || 
        i.status === IncidentStatus.INVESTIGATING
      ).length,
      criticalIncidents: allIncidents.filter(i => 
        i.severity === IncidentSeverity.P1 && 
  ...
  listAlerts(filter?: {status?: string; severity?: string; domain?: string}): Alert[] {
